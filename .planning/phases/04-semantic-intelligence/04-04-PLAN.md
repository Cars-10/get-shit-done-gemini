...
must_haves:
  truths:
    - "Gemini can query 'what uses src/lib/db.ts?' via CLI"
...
<objective>
Expose the orphaned getDependents() function via CLI query interface so Gemini can answer "what uses this file?" questions during sessions.
...
- Output goes to stdout as JSON for Gemini to parse
  </action>
  <verify>
...
All commands should output valid JSON to stdout.
  ...
  <done>
Query actions route to handleQuery(), which returns JSON results via stdout. Gemini can invoke `echo '{"action":"query",...}' | node hooks/gsd-intel-index.js` to query the graph.
  </done>
</task>
...
This makes the CLI interface discoverable for future Gemini sessions.
  </action>
  <verify>
...
- [ ] INTEL-05 requirement unblocked: Gemini can query "what uses this file?"
</success_criteria>
